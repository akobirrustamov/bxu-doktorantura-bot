...
async def start_application(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer("1️⃣ To‘liq F.I.Sh (Familiya, Ism, Sharifingiz) kiriting:", reply_markup=step_keyboard())
...
async def back_to_fullname(callback: types.CallbackQuery):
    await callback.message.answer("1️⃣ Iltimos, F.I.Sh ni qayta kiriting:", reply_markup=step_keyboard())
    await ApplicationStates.full_name.set()
...
async def back_to_phone(callback: types.CallbackQuery):
    await callback.message.answer("2️⃣ Telefon raqamingizni qayta kiriting:", reply_markup=step_keyboard())
...
async def back_to_schedule(callback: types.CallbackQuery):
    await callback.message.answer("3️⃣ Jadval faylini qayta yuboring:", reply_markup=step_keyboard())
...
async def back_to_diploma(callback: types.CallbackQuery):
    await callback.message.answer("4️⃣ Magistrlik diplomini qayta yuboring:", reply_markup=step_keyboard())
...
async def back_to_passport(callback: types.CallbackQuery):
    await callback.message.answer("5️⃣ Pasport faylini qayta yuboring:", reply_markup=step_keyboard())
...
async def back_to_refword(callback: types.CallbackQuery):
    await callback.message.answer("6️⃣ Tavsiyanoma (Word) faylini qayta yuboring:", reply_markup=step_keyboard())
...
async def get_full_name(message: types.Message, state: FSMContext):
    ...
    await message.answer("2️⃣ Telefon raqamingizni (+998901234567) ko‘rinishida yuboring:", reply_markup=step_keyboard())
    await ApplicationStates.phone.set()
...
async def get_phone(message: types.Message, state: FSMContext):
    ...
    await message.answer("3️⃣ Jadval faylini (.doc/.docx/.rtf) yuboring:", reply_markup=step_keyboard())
    await ApplicationStates.schedule_file.set()
...
async def get_schedule_file(message: types.Message, state: FSMContext):
    ...
    await message.answer("4️⃣ Magistrlik diplomini PDF formatda yuboring:", reply_markup=step_keyboard())
    await ApplicationStates.diploma_file.set()
...
async def get_diploma_file(message: types.Message, state: FSMContext):
    ...
    await message.answer("5️⃣ Pasport faylini PDF formatda yuboring:", reply_markup=step_keyboard())
    await ApplicationStates.passport_file.set()
...
async def get_passport_file(message: types.Message, state: FSMContext):
    ...
    await message.answer("6️⃣ Tavsiyanoma (Word) faylini yuboring:", reply_markup=step_keyboard())
    await ApplicationStates.reference_word.set()
...
async def get_reference_word(message: types.Message, state: FSMContext):
    ...
    await message.answer("7️⃣ Tavsiyanoma (PDF) faylini yuboring:", reply_markup=step_keyboard())
